...
func addPartition(cat *catalog.Catalog, cmd *command.Command, db sqlx.DB) error {
...
func createHistoryTableIfNotExists(table *sqlx.Table, db sqlx.DB, users *cache.Users) error {
	...
	q = "CREATE TABLE " + db.TableSQL(table) + " PARTITION OF " + db.HistoryTableSQL(table) + " FOR VALUES IN (TRUE)"
	if _, err := db.Exec(nil, q); err != nil {
		return err
	}
	nctable := "\"" + table.Schema + "\".\"zzz___" + table.Table + "___\""
	q = "CREATE TABLE " + nctable + " PARTITION OF " + db.HistoryTableSQL(table) + " FOR VALUES IN (FALSE) " +
		"PARTITION BY RANGE (__start)"
	if _, err := db.Exec(nil, q); err != nil {
		return err
	}
	// Add indexes on new columns.
	_, err := db.Exec(nil, "CREATE INDEX ON "+db.HistoryTableSQL(table)+" (__id)")
	if err != nil {
		return err
	}
