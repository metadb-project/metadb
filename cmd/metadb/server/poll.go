...
func pollLoop(ctx context.Context, cat *catalog.Catalog, spr *sproc) error {
	...
	*/
	...
	for {
		...
		// Parse
		eventReadCount, err := parseChangeEvents(cat, dedup, consumer, cmdlist, spr.schemaPassFilter,
		...
		// Execute
		if err = execCommandList(ctx, cat, cmdlist, spr.svr.dp, spr.source.Name, syncMode, dedup); err != nil {
...
func parseChangeEvents(cat *catalog.Catalog, dedup *log.MessageSet, consumer *kafka.Consumer, cmdlist *list.List, schemaPassFilter, schemaStopFilter, tableStopFilter []*regexp.Regexp, trimSchemaPrefix, addSchemaPrefix string, sourceFileScanner *bufio.Scanner, sourceLog *log.SourceLog, checkpointSegmentSize int) (int, error) {
