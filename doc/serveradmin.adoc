...
* To build from source, these may have to be installed:
** https://go.dev/[Go] 1.24 or later
** https://www.colm.net/open-source/ragel/[Ragel] 6
** https://gcc.gnu.org/[GCC C compiler] 9 or later
** https://pkg.go.dev/golang.org/x/tools/cmd/goyacc[goyacc] (required
   for Metadb 1.3 or earlier; see installation instructions below)
...
For Metadb 1.3 or earlier, install goyacc:
...
NOTE: In Metadb 1.4, `build.sh` has been renamed to `build`.
...
tables and functions, use the GRANT command (or AUTHORIZE in Metadb
1.3 and earlier).
...
via the command REGISTER USER.  A user schema can then be created with
