...
Metadb can send alerts (notify) to the destination described in the flag "notifierstring" (an SNS topic support at now implemented)
Alert sent when a sync or resync is completed or if the main consuming cycle is broken due to an error.
For SNS implementation: AWS auth configuration cases are described here:
...
[source,bash]
----
metadb start -D data -l metadb.log --notifierstring ARN
----
