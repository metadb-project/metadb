...
----
SELECT id, "desc", "group" FROM user_groups;
----

The Metadb equivalent of this query is:

----
SELECT id, "desc", "group" FROM folio_users.groups__t;
----

The double quotation marks are needed here only because `desc` and `group` are
reserved words in SQL.  Alternatively, they can be removed if the column names
are prefixed with a table alias:

----
SELECT g.id, g.desc, g.group FROM folio_users.groups__t AS g;
----
