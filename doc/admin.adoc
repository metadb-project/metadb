...
Then to begin streaming to Kafka, "POST" a connector configuration to
Kafka Connect at `/connectors`, for example:
...
Note the `1` included in the `name` and `database.server.name`.  This
is suggested as a version number, which can be incremented if the data
stream needs to be resynchronized with a new connector.
...
The replication slot disk usage must be monitored, because in some
cases it can grow too large and fill up the disk.  To show the disk
usage (in the source database):
...
To drop the replication slot (in the source database):
