...
*It is also recommended to allocate plenty of extra disk space in the
source database.*
...
re-streamed data in order to resynchronize with the source, using the
following procedure.
...
2. Stop the Metadb server and (before starting it again) "reset" the
   analytics database to mark all current data as old.  This may take
   some time to run.
+
[source,bash]
----
...
4. Once the new data have finished (or nearly finished) re-streaming,
   stop the Metadb server, and "clean" the analytics database to
   remove any old data that have not been refreshed by the new data
   stream.
+
[source,bash]
----
...
admninistrator, but *it must be run to remove redundant data and to
complete the resynchronization process*.  The only disadvantage of
running it too early is that data will appear to be missing until they
are re-streamed.
