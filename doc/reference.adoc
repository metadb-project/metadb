...
The `external_sql_folio` parameter sets the Git repository and
reference to be used for running FOLIO-specific, external SQL.

For example:

----
alter system set external_sql_folio =
    'https://github.com/folio-org/folio-analytics.git/refs/tags/v1.8.0';
----
...
The `external_sql_reshare` parameter sets the Git repository and
reference to be used for running ReShare-specific, external SQL.

For example:

----
alter system set external_sql_reshare =
    'https://github.com/openlibraryenvironment/reshare-analytics.git/refs/tags/v1.1.0';
----
...
Metadb extends SQL with commands for configuring and administering the
server.  These commands are only available when connecting to the
Metadb server (not the PostgreSQL server for the underlying database).
...
where `*_action_*` is one of:
    ...
    add column `*_column_name_*` `*_data_type_*`
...
Due to concurrency control, `alter table` has to acquire stream
processing or table locks in order to finish executing.  It
