...
Create JSON mappings to transform a "metadata" object and an "identifiers"
array:
...
`inventory\__t__` which includes the "metadata" object columns, using
`metadata` as a prefix in column names, and another table
`inventory\__identifier__` which contains the "identifiers" array data.
