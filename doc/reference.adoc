...
However, if a user already exists but does not have a user schema,
CREATE SCHEMA can be used to create it.
...
Create a schema for an existing user `jane`:
...
[.aqua-background]#Metadb 1.4#
Disable access to tables generated from an external data source
...
The DEAUTHORIZE command revokes access to tables.

.Note
****
[.text-center]
DEAUTHORIZE currently requires restarting the server before it will
take effect.
****
...
|`*_role_specification_*`
|An existing role which will have its authorization revoked.
|===
...
DROP USER removes a database user that is managed by Metadb.
