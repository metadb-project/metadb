...
`PURGE DATA` deletes all data stored in a database object.  The form
`PURGE DATA DROP TABLE` removes specified main tables and their
corresponding current tables.

WARNING: `PURGE DATA DROP TABLE` not only removes a specified main
table but also removes its associated current table.

Dropping tables can take significant time due to concurrency control.
`PURGE DATA` waits until it can safely continue.
...
Remove main tables `+old_1__+` and `+old_2__+` and their corresponding
current tables `+old_1+` and `+old_2+`:

----
PURGE DATA DROP TABLE library.old_1__, library.old_2__;
----
