...
CREATE USER defines a new database user.  In addition, CREATE USER
creates a schema having the same name as the user, if the schema does
not exist, and grants the user privileges on the schema.  The schema
is intended as a workspace for the user.
